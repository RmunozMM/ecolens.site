...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Helpers de formato
  const pct = v => (typeof v === "number" && !Number.isNaN(v)) ? `${(v * 100).toFixed(1)}%` : "No disponible";
  ...
  const safeDate = v => {
    try { return v ? new Date(v).toLocaleString("es-CL") : "No registrada"; }
  ...
  try {
    ...
    // Cálculo de porcentajes con fallback sensato
    const confRouter = (typeof d.conf_router === "number") ? `${(d.conf_router * 100).toFixed(1)}%` : "No disponible";
    // Para experto: si viene null/undefined → No disponible; si 0 numérico → 0.0%
    const confExperto = (d.conf_experto == null) ? "No disponible" : `${(d.conf_experto * 100).toFixed(1)}%`;
    ...
    cont.innerHTML = `
      <div class="detalle-wrapper">
        ...
        <div class="detalle-info">
          ...
          <p><strong>Navegador:</strong> ${txt(d.navegador) !== "No registrado" ? d.navegador : "No identificado"}</p>
          <p><strong>Observador:</strong> ${ (o && (o.nombre || o.usuario)) ? (o.nombre || o.usuario) : "Anónimo" }</p>
    ...
    if (hasCoords(d.latitud, d.longitud)) {
      ...
      L.marker([+d.latitud, +d.longitud]).addTo(map)
        .bindPopup(`<strong>${e.nombre_comun || e.nombre_cientifico || 'Especie detectada'}</strong><br>${txtUbic(d.ubicacion)}`);
    }
...
<style>
.detalle-deteccion { padding: 2rem 1rem; font-family: "Nunito Sans", sans-serif; }
.detalle-wrapper { display: flex; flex-direction: column; gap: 2rem; }
.detalle-imagenes { display: flex; flex-wrap: wrap; gap: 1.5rem; justify-content: flex-start; }
.img-box { flex: 1 1 300px; text-align: center; }
.img-box img { width: 100%; max-width: 400px; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,.15); object-fit: cover; }
.img-label { margin-bottom: .5rem; font-weight: 600; color: #374151; }
.detalle-info { flex: 1; min-width: 280px; }
.detalle-info h2 { margin-top: 0; color: #1f2937; }
.detalle-info .taxon { color: #64748b; font-size: 0.95rem; }
.detalle-info .taxon a { color: #2563eb; text-decoration: none; font-weight: 600; }
.detalle-info .taxon a:hover { text-decoration: underline; }
.descripcion { margin-top: 1rem; line-height: 1.5; color: #374151; }
.mapa { width: 100%; height: 380px; border-radius: 12px; margin-top: 2rem; box-shadow: 0 3px 8px rgba(0,0,0,.2); }
.btn-volver { display: inline-block; margin-top: 2rem; padding: 0.5rem 1rem; background: #3b82f6; color: #fff; border-radius: 8px; text-decoration: none; }
.btn-volver:hover { background: #2563eb; }
@media (min-width: 768px) {
  .detalle-wrapper { flex-direction: row; align-items: flex-start; }
  .detalle-imagenes { flex-direction: column; gap: 1rem; }
}
.det-meta { margin: .5rem 0 1rem; }
.meta-row { display:flex; justify-content:space-between; align-items:center; gap:1rem; }
.badge-id { background:#e5f0ff; color:#1f3b78; padding:.25rem .5rem; border-radius:.5rem; font-weight:700; }
.btn-copy { padding:.25rem .6rem; border:1px solid #d1d5db; border-radius:.5rem; background:#fff; cursor:pointer; }
.btn-copy:hover { background:#f3f4f6; }
.img-box.placeholder .img-ph {
  width:100%; max-width:400px; height:220px; display:flex; align-items:center; justify-content:center;
  border-radius:12px; background:#f3f4f6; color:#6b7280; font-weight:600; border:1px dashed #cbd5e1;
}
.hint-muted { margin-top:1rem; color:#64748b; }
</style>