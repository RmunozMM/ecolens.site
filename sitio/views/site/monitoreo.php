<?php
// Vista: Dashboard de Monitoreo y Telemetría

$this->title = "Dashboard de Monitoreo y Telemetría";
...
// Endpoint de métricas (si lo pones en el env como endpoints['metrics'], lo toma de ahí)
$API_METRICA = $env['endpoints']['metrics'] ?? ($API_BASE . '/api/monitoreo/metrica-general');
?>

<section class="monitoreo-section container">
  <h1>Dashboard de Monitoreo y Telemetría</h1>
  <p class="subtitle">Métricas en tiempo real del desempeño del sistema EcoLens.</p>

  <div id="stats-grid" class="stats-grid"></div>

  <div class="chart-container-grid">
    <div class="chart-item">
      <h2>Latencia de Detección (Últimas 24h)</h2>
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function cargarMetrica() {
    try {
      ...
      if (!data.success) throw new Error(data.message || "Error en respuesta de API");
      ...
      grid.innerHTML = `
        <div class="stat-item"><h3>${data.total_detecciones ?? "—"}</h3><p>Detecciones totales</p></div>
        <div class="stat-item"><h3>${promPrec}%</h3><p>Precisión promedio</p></div>
        <div class="stat-item"><h3>${latProm} ms</h3><p>Latencia promedio</p></div>
        <div class="stat-item"><h3>${data.trl ?? "—"}</h3><p>Madurez tecnológica</p></div>
      `;
      ...
      const topLabels = Array.isArray(data.top_especies) ? data.top_especies.map(x => x.esp_nombre_comun || "Sin nombre") : [];
      const topValues = Array.isArray(data.top_especies) ? data.top_especies.map(x => x.conteo) : [];
