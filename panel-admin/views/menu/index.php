...
<div class="menu-index">
    ...
    <div class="summary">Mostrando <b>1-<?= $totalMenus ?></b> de <b><?= $totalMenus ?></b> elementos.</div>
    ...
    <?php foreach ($menuItems as $menuItem): ?>
        ...
        ?>
        <!-- NIVEL 1 -->
        <tr data-id="<?= $menuItem['nivel_1']->men_id ?>" data-level="1" data-parent="0" class="menu-item">
            <td><?= Html::encode($menuItem['nivel_1']->men_id) ?></td>
            <td style="text-align:center;"><i class="bi <?= Html::encode($iconClass) ?>" style="font-size:1.4em;color:<?= Html::encode($iconColor) ?>;"></i></td>
            ...
            <td class="orden-cell text-center"><i class="fa-solid fa-arrows-alt move-handle"></i></td>
            <td>
                <?= Html::a('<i class="fa-solid fa-eye"></i>', ['menu/view', 'men_id' => $menuItem['nivel_1']->men_id], ['class' => 'btn-action btn-view', 'title' => 'Ver Menú']) ?>
                <?= Html::a('<i class="fa-solid fa-pen"></i>', ['menu/update', 'men_id' => $menuItem['nivel_1']->men_id], ['class' => 'btn-action btn-update', 'title' => 'Actualizar Menú']) ?>
                <?= Html::a('<i class="fa-solid fa-trash"></i>', ['menu/delete', 'men_id' => $menuItem['nivel_1']->men_id], ['class' => 'btn-action btn-delete', 'title' => 'Eliminar Menú', 'data-confirm' => '¿Eliminar este Menú?']) ?>
                <?= Html::a(
                    ...
                    [
                        'class' => 'btn-action btn-visibility ' . ($menuItem['nivel_1']->men_mostrar === 'Si' ? 'visible' : 'oculto'),
                        'title' => ($menuItem['nivel_1']->men_mostrar === 'Si' ? 'Ocultar Menú' : 'Mostrar Menú'),
                        'data-method' => 'post'
                    ]
                ) ?>
            </td>
        ...
        <?php foreach ($menuItem['nivel_2'] as $submenuItem): ?>
            ...
            ?>
            <tr data-id="<?= Html::encode($submenuItem->men_id) ?>" data-level="2" data-parent="<?= Html::encode($submenuItem->men_padre_id) ?>" class="submenu-item">
                <td><?= Html::encode($submenuItem->men_id) ?></td>
                <td style="text-align:center;"><i class="bi <?= Html::encode($iconClass2) ?>" style="font-size:1.2em;color:<?= Html::encode($iconColor2) ?>;"></i></td>
                ...
                <td class="orden-cell text-center"><i class="fa-solid fa-arrows-alt move-handle"></i></td>
                <td>
                    <?= Html::a('<i class="fa-solid fa-eye"></i>', ['menu/view', 'men_id' => $submenuItem->men_id], ['class' => 'btn-action btn-view', 'title' => 'Ver Menú']) ?>
                    <?= Html::a('<i class="fa-solid fa-pen"></i>', ['menu/update', 'men_id' => $submenuItem->men_id], ['class' => 'btn-action btn-update', 'title' => 'Actualizar Menú']) ?>
                    <?= Html::a('<i class="fa-solid fa-trash"></i>', ['menu/delete', 'men_id' => $submenuItem->men_id], ['class' => 'btn-action btn-delete', 'title' => 'Eliminar Menú', 'data-confirm' => '¿Eliminar este Menú?']) ?>
                    <?= Html::a(
                        ...
                        [
                            'class' => 'btn-action btn-visibility ' . ($submenuItem->men_mostrar === 'Si' ? 'visible' : 'oculto'),
                            'title' => ($submenuItem->men_mostrar === 'Si' ? 'Ocultar Menú' : 'Mostrar Menú'),
                            'data-method' => 'post'
                        ]
                    ) ?>
                </td>
